...
async function handleRepProfile(request, env) {
  ...
  const legislator = await env.DB.prepare(`
    SELECT
      ...
      people.citizens_count_url,
      people.state_house_profile_url,
      people.substack_url,
...
function isTpActionAlignedSelectExpression(column) {
  return `
    CASE
      WHEN LOWER(CAST(COALESCE(${column}, '') AS TEXT)) IN ('1', 'true', 'yes', 'y')
      THEN 'yes'
      ELSE 'no'
    END AS is_tpaction_aligned_2026
  `;
}
  ...
  return `
    ...
    candidate_base AS (
      SELECT
        ...
        p.ballotpedia_url AS ballotpedia_url,
        p.citizens_count_url AS citizens_count_url,
        p.state_house_profile_url AS state_house_profile_url,
        p.substack_url AS substack_url,
        p.instagram_url AS instagram_url,
        p.facebook_url AS facebook_url,
        p.tiktok_url AS tiktok_url,
        p.x_url AS x_url,
...
function formatCandidate(candidate) {
  return {
    ...
    ballotpediaUrl: candidate.ballotpedia_url,
    ballotpedia_url: candidate.ballotpedia_url,
    citizensCountUrl: candidate.citizens_count_url,
    citizens_count_url: candidate.citizens_count_url,
    stateHouseProfileUrl: candidate.state_house_profile_url,
    state_house_profile_url: candidate.state_house_profile_url,
    substackUrl: candidate.substack_url,
    substack_url: candidate.substack_url,
    instagramUrl: candidate.instagram_url,
    instagram_url: candidate.instagram_url,
    facebookUrl: candidate.facebook_url,
    facebook_url: candidate.facebook_url,
    tiktokUrl: candidate.tiktok_url,
    tiktok_url: candidate.tiktok_url,
    xUrl: candidate.x_url,
    x_url: candidate.x_url,
    blueskyUrl: candidate.bluesky_url,
