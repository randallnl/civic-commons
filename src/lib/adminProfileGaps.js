...
async function getRepresentativeGaps(db, { missing, office, limit }) {
  ...
  return (result.results || []).map((row) => {
    ...
    return {
      ...
      key: row.personid,
      name,
      photoUrl: hasUsefulPhoto(row.photo_url) ? cleanText(row.photo_url) : "",
      party: cleanText(row.party || ""),
      office: row.legislativebody === "S" ? "State Senator" : "State Representative",
      chamber: row.legislativebody === "S" ? "Senate" : "House",
